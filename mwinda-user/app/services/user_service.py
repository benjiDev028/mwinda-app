...
async def register_admin(db: AsyncSession, user: UserCreate) -> UserResponse:
    try:
        result = await db.execute(select(User).where(User.email == user.email))
        existing_user = result.scalar_one_or_none()
        if existing_user:
...
async def Resend_activation_email(db: AsyncSession, email: str):
    try:
        result = await db.execute(select(User).where(User.email == email))
        user = result.scalar_one_or_none()

        if not user:
            logging.warning("User not found for resending activation email: %s", email)
            raise HTTPException(status_code=404, detail="Utilisateur non trouvé.")
        ...
        logging.info("Activation email resent successfully to: %s", email)
        return {"message": "Activation email resent"}

    except HTTPException:
        raise
    except Exception as e:
        logging.error("Error resending activation email: %s", str(e))
        raise HTTPException(status_code=500, detail="Internal server error.")
